...
const ListView = () => {
  ...
  const [dataDate, setdataDate] = useState<any>("-1");

  const router = useRouter();
  const [parentid, setparentid] = useState("0");
  const [add, setadd] = useState("0");
  const [view, setview] = useState("0");
  const [valdate, setvaldate] = useState<any>("0");
  useEffect(() => {
  ...
  useEffect(() => {
    var backDate = new Date(
      new Date(datalocal?.data?.bussinesDate).getTime() -
        1 * 24 * 60 * 60 * 1000
    )
      .toISOString()
      .split("T")[0];
    console.log("testdate", backDate);
    setvaldate(backDate);

    setdataDate(backDate);
  }, []);
  return (
    <>
      <Seo
