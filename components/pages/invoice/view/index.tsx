...
const InvoiceView = () => {
  const FOLDER = "accounting/invoice";
  const FOLDERUPDATESTATUS = "/cms/accounting/invoice/update-status";
  const {canCreate, canUpdate} = useFormPermission(1039);
  ...
  const canProcess = useTransactionPermission("processed_invoice");
  const canCancel = useTransactionPermission("cancel_invoice");
    ...
    try {
      ...
      if (saveprocess?.code == "200") {
        ...
        router.push({
          ...
          query: { parent: GetQueryStr("parent") , view: "1", data: GetQueryStr("data"), module: GetQueryStr("module")},
        });
      } else {
        setloading(false);
      }
