...
const AddView = (props: AddviewProps) => {
  ...
  const GetDetailUser = async (i: any) => {
    ...
    try {
      ...
      dataInput[0].data[2].options = datauser?.master?.roomTypes;
      dataInput[0].data[4].options = datauser?.master?.languages;
  ...
  const transformData = (data) => {
    ...
    const propertiesToTransform = ["status", "language", "room_type_id"];
  ...
  const OnSave = async () => {
    ...
    try {
      ...
      formData.append("group", "seo-room");
      ...
      toast(mth === "POST" ? "Success save banner" : "Success edit banner", {
        autoClose: 3000,
        type: "success",
