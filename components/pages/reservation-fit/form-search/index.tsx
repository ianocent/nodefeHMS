...
const AddView = (props: AddviewProps) => {
  ...
  const [IsChangeToReservation, setIsChangeToReservation] = useState(false);
  const { canCreate, canUpdate } = useFormPermission(61);
      ...
      items: [
        {
          data: [
            ...
            },
            {
              ...
              idpost: "room_type_id",
              relate: "0;2",
              uri: "/cms/room-type?reservation=1&module=search&check_in_date=[0]&check_out_date=[1]",
              sugestdata: "a",
              value: "",
              valueid: "",
              required: true,
    ...
    dataInput[1].items?.push({
      data: [
        ...
        },
        {
          ...
          placeholder: "Search Rate Here.",
          idpost: "rate_id",
          relate: "room_type_id;check_in_date;check_out_date",
  ...
  const GetRateList = async (word, uri, relate: any, ix, ia) => {
    try {
      setloading(true);
      let getuuri = "";
      if (relate) {
  ...
  const ListTblRate = (id, datI, name, ix, ia, isAdd) => {
    return (
      <>
        <div ref={ref} className="p-2 rounded-md w-full z-50 bg-white">
          <>
            <div className="table-responsive w-full">
              <table className={"shadow-lg table-auto w-full"}>
                <thead>
                  <tr className="">
                    {dataRate?.table?.map((row: any, i: any) => (
                      <td
                        title={"Sort By " + row.label}
                        key={i}
                        className="bg-[#323A50] text-white p-2 font-bold cursor-pointer"
                ...
                <tbody>
                  {dataRate?.data?.map((row: any, index) => (
                    <>
                      <tr
                        key={row?.id + "-" + index}
                        className={
                          ...
                          (activeRate == row ? "bg-[#d4e4fc]" : "")
                        }
                      >
                        {dataRate?.table?.map((item: any, a: any) => {
                          return item.row != 1 ? (
                            <td
                              key={item.key + "-" + a}
                              onClick={() => {
          ...
          <div className="col-span-12 grid grid-cols-12 h-fit  gap-2">
            <div className="col-span-8 ">
              <fieldset className="border">
                <legend className="">
                  {IsChangeToReservation ? "Guest Profile" : "Company Profile"}
                </legend>
                <div className="grid grid-cols-12 h-fit gap-2 ml-2 mb-4 mt-4 mr-2">
                  {dataform[0].data?.map((row: any, index) => (
                    <>
                      {(row?.type != "hidden" && dataInputs.length == 0) ||
                      inArray(row?.name, dataInputs) ? (
              ...
              {dataform[1].items?.map((rw: any, i) => (
                <>
                  ...
                  {popuprate ? (
                    <>
                      <fieldset className="border min-w-full table-auto">
                        <legend className="">List Rate</legend>
                        {ListTblRate("rate_id", 0, "name-rate", 8, 0, false)}
                      </fieldset>
                    </>
                  ) : (
                    <></>
            ...
            <div className="col-span-4 ">
              <div>
                {IsChangeToReservation || activeRate ? (
                  <>
                    <div className="mb-2 ">
            ...
            <>
              <ButtonSubmit
                onCreate={() => {
                  GetRateList(
                    "",
                    "/cms/reservation/rate-by-company-id?company_profile_id=[0]&check_in_date=[1]&check_out_date=[2]",
                    "company_profile_id;0;2",
                    8,
                    0
                  );
                  // GetRate();
