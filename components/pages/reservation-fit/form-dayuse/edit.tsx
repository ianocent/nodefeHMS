...
const EditView = () => {
  ...
  const changeHandlera = (
    ...
    if (
      ...
      if (e.target.name == "quantity") {
        dataInput[form].items[index].data[ia].triggerChange = true;
      }
      ...
      if (data?.code == "200") {
        ...
        data?.data?.reservation_items?.map((rws, index) => {
          ...
          dataform[2].items[index]?.data?.map((rw, i) => {
            ...
            } else if (rw?.name == "name-package") {
              dataInput[2].items[index].data[i].valueOri =
                data?.data?.reservation_items[index]?.package_id;
              dataInput[2].items[index].data[i].value =
                data?.data?.reservation_items[index]?.package_id?.label;
              dataInput[2].items[index].data[i].valueid =
                data?.data?.reservation_items[index]?.package_id?.value;
        ...
        if (data?.data?.special_instruction?.remark_ins) {
  ...
  const FinalPOstDat = () => {
    ...
    objpost.check_out_date =
      dataform[2].items[dataform[2].items.length - 1].data[9].value;
    objpost.sub_folio_selected = bulkData;
    objpost.type_reservation = "day-use";
    return objpost;
  };
  ...
  const onSelecteda = (
    ...
    if (name == "name-rate") {
      dataInput[frm].items[ia].data[4].uri =
        "/cms/room-type?reservation=1&check_in_date=[0]&check_out_date=[1]&rate_id=" +
        rw?.id;
      dataInput[frm].items[ia].data[18].valueOri = {};
      dataInput[frm].items[ia].data[18].value = "";
      dataInput[frm].items[ia].data[18].valueid = "";

      dataInput[frm].items[ia].data[19].value = "0";
    }

    if (name == "name-package") {
      dataInput[frm].items[ia].data[18].valueOri.time = rw?.time?.value;
    }
        ...
        <div className=" m-4">
          <div className="p-2 font-bold">
            <h1 className="capitalize">
              {namePopUp == "first_name-guest" ? "Guest Profil" : "Company"}
            </h1>
          </div>
  ...
  useEffect(() => {
    if (
      dataform[2].items[0].data[18].valueOri?.time &&
      dataform[2].items[0].data[19].triggerChange
    ) {
      setisChangeCharge(true);
      GetRate();
    }
  }, [
    dataform[2].items[0].data[19].value,
    dataform[2].items[0].data[18].valueOri?.time,
  ]);
            ...
            <div className="col-span-3 ">
              ...
              <div className="mt-2">
                <button
                  ...
                  // className="mb-2 px-4 py-2 bg-primary text-white rounded-sm hover:bg-cyan hover:text-black transition"
                >
                  <span
                ...
                {datadetail?.guest?.guest_profile_id && showSections.guestPreference && (
                  <TableView
                    uri="/cms/profile/guest-preference"
                    queryString={"&guest_id=" + datadetail.guest.guest_profile_id}
                    groups=""
                    isEditTable={true}
                    isTitle={true}
                    isDeleted={false}
              ...
              <div className="mt-2 mb-2">
                ...
                {datadetail?.guest?.guest_profile_id && showSections.guestNotes && (
                  <TableView
